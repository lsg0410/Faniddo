...
public class SignUpActivity extends Activity {
    ...
    int id_flag = 0;
    TextView signup_id, signup_password,signup_name,signup_email;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        signup_id = (TextView) findViewById(R.id.signup_id);
        signup_password = (TextView) findViewById(R.id.signup_password);
        signup_name = (TextView) findViewById(R.id.signup_name);
        signup_email = (TextView) findViewById(R.id.signup_email);

        temp1 = this.getResources().getDrawable(R.drawable.profile);
        ...
        signup_btn.setOnClickListener(new Button.OnClickListener() {
            ...
            public void onClick(View v) {
                temp = imageView.getDrawable();
                tmpBitmap = ((BitmapDrawable)temp).getBitmap();
                if(id_flag == 0){
                    Toast.makeText(SignUpActivity.this, "아이디 중복확인을 해주세요", Toast.LENGTH_SHORT).show();
                }else if(signup_id.getText().toString().length() == 0 ||signup_password.getText().toString().length() == 0 || signup_name.getText().toString().length() == 0 || signup_email.getText().toString().length() == 0){
                    Toast.makeText(SignUpActivity.this, "정보를 전부 입력해주세요", Toast.LENGTH_SHORT).show();
                }else if(tmpBitmap.equals(tmpBitmap1)){
                    MemberVO vo = new MemberVO();
                    vo.setMid(signup_id.getText().toString());
                    vo.setMpwd(signup_password.getText().toString());
                    vo.setMname(signup_name.getText().toString());
                    vo.setMemail(signup_email.getText().toString());

                    MemberService service = MemberService.retrofit.create(MemberService.class);
                    Call<ResponseBody> call = service.noimgsignup(vo);
                    ...
                    MemberService service = MemberService.retrofit.create(MemberService.class);
                    Call<ResponseBody> call = service.signup(body,vo);
                    call.enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            finish();
        ...
        id_check.setOnClickListener(new Button.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(signup_id.getText().toString().length() == 0){
                    ...
                    MemberService service = MemberService.retrofit.create(MemberService.class);
                    Call<ResponseBody> call = service.idcheck(signup_id.getText().toString());
                    call.enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
        ...
        addimg.setOnClickListener(new ImageButton.OnClickListener() {
            @Override
            public void onClick(View v) {
