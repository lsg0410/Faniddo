...
import static com.example.yjkim.faniddo.R.id.imageView;
...
public class BoardActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mtoolbar.setTitle("Faniddo 일반게시글 작성");

        Intent intent = getIntent();
        userdata = intent.getStringExtra("writer");

        imageView = (ImageView) findViewById(R.id.imageView);
        btitle = (TextView) findViewById(R.id.btitle);
        bcontent = (TextView) findViewById(R.id.bcontent);
        Button board_register = (Button) findViewById(R.id.board_register);

        temp1 = this.getResources().getDrawable(R.drawable.noimage);
        tmpBitmap1 = ((BitmapDrawable) temp1).getBitmap();

        board_register.setOnClickListener(new Button.OnClickListener() {
            @Override
            public void onClick(View v) {
                temp = imageView.getDrawable();
                tmpBitmap = ((BitmapDrawable) temp).getBitmap();

                if (btitle.getText().toString().length() == 0 || bcontent.getText().toString().length() == 0) {
                    Toast.makeText(BoardActivity.this, "제목과 내용을 전부 작성해주세요.", Toast.LENGTH_SHORT).show();
                } else if (tmpBitmap.equals(tmpBitmap1)) {
                    BoardVO vo = new BoardVO();
                    vo.setBtitle(btitle.getText().toString());
                    vo.setBcontent(bcontent.getText().toString());
                    vo.setBwriter(userdata);

                    BoardService service = BoardService.retrofit.create(BoardService.class);
                    Call<ResponseBody> call = service.noimgregister(vo);
                    ...
                    BoardService service = BoardService.retrofit.create(BoardService.class);
                    Call<ResponseBody> call = service.register(body, vo);
                    call.enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            finish();
        ...
        switch (requestCode) {
            case REQUEST_TAKE_PHOTO:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Toast.makeText(BoardActivity.this, "사진찍기를 취소하였습니다.", Toast.LENGTH_SHORT).show();
                }
                break;
        ...
        switch (requestCode) {
            case MY_PERMISSION_CAMERA:
                for (int i = 0; i < grantResults.length; i++) {
                    ...
                    if (grantResults[i] < 0) {
                        Toast.makeText(BoardActivity.this, "해당 권한을 활성화 하셔야 합니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
                // 허용했다면 이 부분에서..
